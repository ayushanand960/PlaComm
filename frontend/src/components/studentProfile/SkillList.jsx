...
export default function SkillList() {
  ...
  const fetch = async () => {
    try {
      const res = await axiosInstance.get("/students/skills/");
      setList(res.data);
    } catch (e) {
      console.error(e);
    }
  ...
  const handleOpen = (item = null) => {
    setEditing(item);
    setForm(item ? { name: item.name, level: item.level } : { name: "", level: "" });
    setOpen(true);
  };
  ...
  const handleSubmit = async () => {
    try {
      if (editing) {
        // ✅ correct endpoint for editing skill
        await axiosInstance.patch(`/students/skills/${editing.id}/`, form);
      } else {
        // ✅ correct endpoint for adding new skill
        await axiosInstance.post("/students/skills/", form);
      }
      fetch();
      handleClose();
    } catch (e) {
        ...
        {list.map((s) => (
          <Grid item xs={12} md={4} key={s.id}>
            <Card>
              <CardContent>
                <Typography variant="h6">{s.name}</Typography>
                <Typography color="text.secondary">{s.level}</Typography>
              </CardContent>
              <CardActions>
                <Button onClick={() => handleOpen(s)}>Edit</Button>
