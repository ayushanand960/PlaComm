...
//                 <TableHead sx={{ backgroundColor: "#E3F2FD" }}>
//                   <TableRow>
//                     {["#", "Name", "Email", "RUM No", "Phone"].map((head, idx) => (
//                       <TableCell
//                         key={idx}
//                         sx={{
//                           fontWeight: "bold",
...
const AdminJobApplicants = () => {
  ...
  const fetchCompanyDetails = async () => {
    try {
      const response = await axiosInstance.get(`placements/company-details/${id}/`);
      setCompanyData(response.data);
    } catch (error) {
      console.error("Error fetching company details:", error);
    } finally {
    ...
    <Box sx={{ p: { xs: 2, md: 4 } }}>
      ...
      {companyData.jobs?.map((job, jobIndex) => (
        ...
        >
          ...
          {/* ✅ Job Description above the table */}
          <Box
            sx={{
              mb: 3,
              p: 2,
          ...
          >
            ...
            >
              <WorkOutlineIcon fontSize="small" /> Job Description
            </Typography>
            <Typography
              variant="body1"
              color="textSecondary"
